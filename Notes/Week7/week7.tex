...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \emph{meta}-chloronitrobenzene: No reaction.
    \end{itemize}
    \item Mechanism.
    \begin{figure}[h!]
        \centering
        ...
        \schemestart
            ...
            \chemleft{[}
                \subscheme{
                    \chemfig{*6(-(-[@{sb3a}]\charge{45:1pt=$\oplus$}{N}(=[@{db3a}:-150]@{O3}O)(-[:-30]\charge{45:1pt=$\ominus$}{O}))=[@{db3b}]-[@{sb3b}]@{C3}\charge{45:1pt=$\ominus$}{}-[@{sb3c}](-[:70]Nu)(-[@{sb3d}:110]@{Cl3}Cl)-=)}
                    \arrow{<->}
        ...
        \chemmove{
            ...
            \draw [rex,semithick,shorten <=10pt,shorten >=2pt] (C3) to[bend right=90,looseness=5] (sb3c);
            \draw [rex,semithick,shorten <=2pt,shorten >=2pt] (sb3d) to[bend left=90,looseness=3] (Cl3);
            \draw [blx,semithick,shorten <=8pt,shorten >=2pt] (C3) to[bend left=90,looseness=5] (sb3b);
            \draw [blx,semithick,shorten <=2pt,shorten >=2pt] (db3b) to[bend left=45,looseness=1.5] (sb3a);
            \draw [blx,semithick,shorten <=3pt,shorten >=2pt] (db3a) to[bend right=90,looseness=3] (O3);
        }
    ...
    \begin{itemize}
        ...
        \item This is similar to dissolving metal reduction of alkynes.
    \end{itemize}
    \item Mechanism.
    \begin{figure}[h!]
        \centering
    ...
    \end{figure}
    \begin{itemize}
        \item Although we draw the lithium radical directly attacking benzene, in reality, lithium gives up one of its electrons to become a cation, and this electron is solvated by \ce{NH3}. Think \textbf{electrides} from \textcite{bib:CHEM20100Notes}!
    \end{itemize}
    ...
    \begin{figure}[h!]
        ...
        \begin{tikzpicture}
            \node{
                \chemnameinit{}
                \schemestart
                    \chemfig{*6(=-(-[@{sb1a}]@{H1}H)=[@{db1}](-[@{sb1b}]@{Br1}Br)-=-)}
        ...
        \chemmove{
            ...
            \draw [rex,semithick,shorten <=2pt,shorten >=2pt] (sb1a) to[bend right=60,looseness=1.5] (db1);
            \draw [rex,semithick,shorten <=2pt,shorten >=2pt] (sb1b) to[bend left=90,looseness=3] (Br1);
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item To finish the synthesis, just chlorinate \emph{para} (\ce{Cl2 / AlCl3}) and reduce the nitro groups (\ce{H2 + Pd/C}).
    \end{itemize}
